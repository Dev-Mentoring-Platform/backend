...
public class InitService {
    ...
    void init() {
        ...
        // user / mentee
        User user1 = loginService.signUp(getSignUpRequestWithNameAndZone("user1", "부산광역시 기장군 내리"));
        Mentee mentee1 = loginService.verifyEmail(user1.getUsername(), user1.getEmailVerifyToken());

        User user2 = loginService.signUp(getSignUpRequestWithNameAndZone("user2", "서울특별시 종로구 효자동"));
        Mentee mentee2 = loginService.verifyEmail(user2.getUsername(), user2.getEmailVerifyToken());

        User user3 = loginService.signUp(getSignUpRequestWithNameAndZone("user3", "경상북도 영주시 영주동"));
        Mentee mentee3 = loginService.verifyEmail(user3.getUsername(), user3.getEmailVerifyToken());

        User user4 = loginService.signUp(getSignUpRequestWithNameAndZone("user4", "부산광역시 금정구 금사동"));
        Mentee mentee4 = loginService.verifyEmail(user4.getUsername(), user4.getEmailVerifyToken());

        User user5 = loginService.signUp(getSignUpRequestWithNameAndZone("user5", "경상남도 진주시 망경동"));
        Mentee mentee5 = loginService.verifyEmail(user5.getUsername(), user5.getEmailVerifyToken());

        Mentor mentor1 = mentorService.createMentor(user4, getMentorSignUpRequestWithJobAndCompanyNameAndEducationLevelAndSchoolNameAndMajor("engineer", "company1", EducationLevelType.HIGH, "school1", "computer"));
        ...
        // review
        Review parent1 = reviewService.createMenteeReview(user1, lecture1.getId(), getMenteeReviewCreateRequestWithScoreAndContent(5, "좋아요"));
        Review child1 = reviewService.createMentorReview(user4, lecture1.getId(), parent1.getId(), getMentorReviewCreateRequestWithContent("감사합니다!"));

        Review parent2 = reviewService.createMenteeReview(user2, lecture1.getId(), getMenteeReviewCreateRequestWithScoreAndContent(3, "별로에요"));
        Review child2 = reviewService.createMentorReview(user4, lecture1.getId(), parent2.getId(), getMentorReviewCreateRequestWithContent("아쉽네요!"));

        Review parent3 = reviewService.createMenteeReview(user1, lecture2.getId(), getMenteeReviewCreateRequestWithScoreAndContent(1, "환불했어요"));
        Review child3 = reviewService.createMentorReview(user4, lecture2.getId(), parent3.getId(), getMentorReviewCreateRequestWithContent("죄송합니다"));
    }
