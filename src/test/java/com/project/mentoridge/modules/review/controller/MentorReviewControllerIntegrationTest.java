...
class MentorReviewControllerIntegrationTest extends AbstractControllerIntegrationTest {
    ...
    void get_paged_my_reviews_written_by_my_mentees() throws Exception {
        ...
        mockMvc.perform(get(BASE_URL + "/by-mentees")
                ...
                // child
                .andExpect(jsonPath("$.content[1].child").doesNotExist())
                // lecture
                ...
                .andExpect(jsonPath("$.content[1].lecture.content").value(lecture.getContent()))
                .andExpect(jsonPath("$.content[1].lecture.difficulty").value(lecture.getDifficulty().name()))
